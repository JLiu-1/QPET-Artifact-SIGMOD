...
void sperr::SPECK3D_VEC_FLT::m_instantiate_encoder()
{
  switch (m_uint_flag) {
    case UINTType::UINT8:
...
void sperr::SPECK3D_VEC_FLT::m_instantiate_decoder()
{
  switch (m_uint_flag) {
    case UINTType::UINT8:
...
void sperr::SPECK3D_VEC_FLT::m_wavelet_xform()
{
  //for(auto i:{0,1,2})
    m_cdf.dwt3d();
}

void sperr::SPECK3D_VEC_FLT::m_inverse_wavelet_xform(bool multi_res)
{
  //for(auto i:{0,1,2}){
    if (!multi_res)
      m_cdf.idwt3d();
