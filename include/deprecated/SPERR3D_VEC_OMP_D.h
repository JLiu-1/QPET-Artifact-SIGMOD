...
#ifndef SPERR3D_VEC_OMP_D_H
#define SPERR3D_VEC_OMP_D_H
...
class SPERR3D_OMP_D {
 public:
  ...
  auto use_bitstream(const void*, size_t,const void*, size_t,const void*, size_t) -> RTNType;

  // The pointer passed in here MUST be the same as the one passed to `use_bitstream()`.
  auto decompress(const void* bitstream1,const void* bitstream2,const void* bitstream3, bool multi_res = false) -> RTNType;

  auto view_decoded_data() const -> std::array<const sperr::vecd_type&,3>;
  //auto view_hierarchy() const -> const std::vector<vecd_type>&;
  auto release_decoded_data() -> std::array<sperr::vecd_type&&,3>;//????
 ...
 private:
  ...
  //
  std::vector<std::unique_ptr<SPECK3D_VEC_FLT>> m_decompressors;
#else
  ...
  //
  std::unique_ptr<SPECK3D_VEC_FLT> m_decompressor;
#endif

  std::array<sperr::vecd_type,3> m_vol_buf;
  //std::vector<vecd_type> m_hierarchy;  // multi-resolution decoding
  std::array<std::vector<size_t>,3> m_offsets;       // Address offset to locate each bitstream chunk.
  std::array<const uint8_t*,3> m_bitstream_ptr = nullptr;
